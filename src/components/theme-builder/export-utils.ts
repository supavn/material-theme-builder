...
export function exportToJSON(theme: ThemeExport): void {
  ...
  let themeName = 'material-theme';
  if (theme.description) {
    const match = theme.description.match(/TYPE:\s*(\w+)/);
    themeName = match ? match[1].toLowerCase() + '-theme' : 'custom-theme';
  }
...
function generateDartCode(theme: ThemeExport): string {
  ...
  // Extract theme name from description or use default
  let themeName = 'Custom Material Theme';
  if (theme.description) {
    const match = theme.description.match(/TYPE:\s*(\w+)/);
    themeName = match ? `${match[1]} Material Theme` : 'Custom Material Theme';
  }
...
class ${(getThemeNameFromExport(theme) || 'Custom').replace(/\s+/g, '')}Theme {
